        ...
        $("select[name='RuleCondition["+index+"][condition_value]'] optgroup").each(function() {
            if(condition_val == $(this).attr('label')){
                ...
                if(condition_value_optgroup_label != $(element).attr('label')){
                    //alert('$(element).children().first().attr(value) ' + $(element).children().first().attr('value'));
                    
                    // check first if option does exists
                    if($(element).find('option[value="' + condition_value_val + '"]').length > 0) {
                        $(element).find('option[value="' + condition_value_val + '"]').prop('selected', true);
                    }else {
        ...
        $("select[name='RuleAction["+index+"][action_value]'] optgroup").each(function() {
            if(action_val == $(this).attr('label')){
                ...
                if(action_value_optgroup_label != $(element).attr('label')){
                    // check first if option does exists
                    if($(element).find('option[value="' + action_value_val + '"]').length > 0) {
                        $(element).find('option[value="' + action_value_val + '"]').prop('selected', true);
                    }else {
                        $(element).children().first().prop('selected', true);
                    }
