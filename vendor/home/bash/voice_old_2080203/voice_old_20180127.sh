...
function record {
    ...
    # record only when there is sound, and cuts out the silence and no gaps and no long silences
    #cmd="sox -t alsa default stt.flac channels 1 rate 16000 silence 1 0.1 5% 1 1.0 5% "
    cmd="sox -t alsa default stt.flac channels 1 rate 16000"
    # 1e parameter is trim
    cmd+=" $1"
...
function record_threshold {
    ...
    while true; do
        ...
        info=$(sox -t .flac stt.flac -n stat 2>&1)
        #echo "$info"
        max_amp=$(echo "$info" | awk '/^Maximum\ amplitude/ {print int($3 * 1000)}')
        min_amp=$(echo "$info" | awk '/^Minimum\ amplitude/ {print int($3 * 1000)}')
