...
class home_service:
    ...
    def start(self):
        ...
        if not self.status():
            subprocess.call(['sudo', 'systemctl', 'start', 'home.service']) # The call function from the subprocess module. This is basically just like the Popen class and takes all of the same arguments, but it simply waits until the command completes and gives you the return code.
    
    def status(self):
        if not subprocess.call(['sudo', 'systemctl', 'status', 'home.service']):
    ...
    def stop(self):
        ...
        if self.status():
            subprocess.call(['sudo', 'systemctl', 'stop', 'home.service'])
